...
class PointCloudGenerator(object):
    ...
    @param sim:       MuJoCo simulation object (robosuite MjSim), env object with .sim attribute,
                      or a callable that returns the sim object.
                      For robosuite 1.5.x compatibility with hard_reset=True, pass the env object
                      instead of env.sim, as reset() creates a new sim object.
    ...
    def __init__(self, sim, cam_names:List, img_size=84):
        ...
        # 1. Direct sim object (MjSim)
        # 2. Environment object with .sim attribute (for robosuite 1.5.x hard_reset compatibility)
        # 3. Callable that returns sim object
        if callable(sim):
            self._sim_getter = sim
            self._env = None
        ...
        # List of camera intrinsic matrices (computed lazily)
        self._cam_mats = None

    @property
    def sim(self):
        """Get the current sim object, handling all input types."""
        if self._sim_getter is not None:
            return self._sim_getter()
        elif self._env is not None:
            return self._env.sim
        else:
            return self._direct_sim

    @property
    def cam_mats(self):
        """Lazily compute camera matrices using current sim."""
        if self._cam_mats is None:
            self._cam_mats = []
            model = self.sim.model
            for idx in range(len(self.cam_names)):
                cam_id = model.camera_name2id(self.cam_names[idx])
    ...
    def generateCroppedPointCloud(self, save_img_dir=None, device_id=0):
        ...
        for cam_i in range(len(self.cam_names)):
            ...
            cam_body_id = sim.model.cam_bodyid[cam_i]
            cam_pos = sim.model.body_pos[cam_body_id]
            c2b_r = rotMatList2NPRotMat(sim.model.cam_mat0[cam_i])
    ...
    def captureImage(self, camera_name, capture_depth=True, device_id=0):
        sim = self.sim
        rendered_images = sim.render(self.img_width, self.img_height, camera_name=camera_name, depth=capture_depth, device_id=device_id)
        if capture_depth:
