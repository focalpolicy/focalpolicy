...
def sample_two_timesteps(cfg, num_samples: int, device: torch.device, eps=1e-5):
    if cfg["tr_sampler"] == "v0":
        t = torch.rand(num_samples, device=device) * (1 - eps) + eps # 1=sde.T
        _, r = sample_two_timesteps_t_r_v0(cfg, num_samples, device=device)
        return t, r
    elif cfg["tr_sampler"] == "v1":
        t = torch.rand(num_samples, device=device) * (1 - eps) + eps # 1=sde.T
        _, r = sample_two_timesteps_t_r_v1(cfg, num_samples, device=device)
        return t, r
    else:
