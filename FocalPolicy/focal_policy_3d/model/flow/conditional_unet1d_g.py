...
from focal_policy_3d.model.flow.conv1d_components import (
    Downsample1d, Upsample1d, Conv1dBlock)
from focal_policy_3d.model.flow.positional_embedding import SinusoidalPosEmb
...
class ConditionalUnet1D(nn.Module):
    def __init__(self, 
        ...
        # number of effective downsample stages (controls required length multiple)
        self.num_downsamples = sum(1 for (_, _, d) in self.down_modules if not isinstance(d, nn.Identity))
        self.downsample_factor = 2 ** self.num_downsamples
    ...
    def forward(self, 
        ...
        # orig_len = sample.shape[-1]
        # # pad time length to be divisible by 2^num_downsamples to avoid off-by-one
        # pad_len = 0
        # if self.downsample_factor > 1:
        #     rem = orig_len % self.downsample_factor
        #     if rem != 0:
        #         pad_len = self.downsample_factor - rem
        # if pad_len > 0:
        ...
        if local_cond is not None:
            ...
            # if pad_len > 0:
            #     local_cond = F.pad(local_cond, (0, pad_len))
        ...
        for idx, (resnet, resnet2, upsample) in enumerate(self.up_modules):
            # skip = h.pop()
            # # align temporal length before concatenation
        ...
        # # crop back to original temporal length if we padded
        # if pad_len > 0:
        #     x = x[..., :orig_len]
